class App {
	constructor(questions) {
		this.questions = questions.map((q, index) =>
			q.multi
				? new MultiChoiceQuestion(q, index)
				: new SingleChoiceQuestion(q, index)
		);
	...
	showResults() {
		...
		this.questions.forEach((currentQuestion, questionNumber) => {
			...
			if (currentQuestion.multi) {
				const selectedCheckboxes = answerContainer.querySelectorAll(
					`input[name=question-${questionNumber}]:checked`
				);

				let selectedValues = '';
				Array.from(selectedCheckboxes).forEach(
					(checkbox) => (selectedValues += checkbox.value)
				);
